...
namespace Codenation.Challenge
{
    ...
    {      
        ...
        [Fact]
        public void Should_Get_Name_Team() {
            var manager = new SoccerTeamsManager();
            manager.AddTeam(1, "Time 1", DateTime.Now, "cor 1", "cor 2");
            Assert.Equal("Time 1",manager.GetTeamName(1));
            Assert.Throws<TeamNotFoundException>(() =>
               manager.GetTeamName(90));
        }
        [Fact]
        public void Should_Get_Name_Player() {
            var manager = new SoccerTeamsManager();
            manager.AddTeam(1, "Time 1", DateTime.Now, "cor 1", "cor 2");

            manager.AddPlayer(50, 1, $"CR7", DateTime.Today, 0, 0);


            Assert.Equal("CR7", manager.GetPlayerName(50));
            Assert.Throws<PlayerNotFoundException>(() =>
                manager.GetPlayerName(25));
        }
       /* [Fact]
